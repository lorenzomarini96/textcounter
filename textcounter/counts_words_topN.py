...
Program that counts the relative frequencies of TOP N words in all text in 
...
def counter_words(file_path, N):
    ...
    N         : ranking number (the "N" most frequent)
    ...
    # TOP N
    words_dictonary_top_N = dict(list(words_dictonary.items())[:N])
        
    #for word, value in words_dictonary_top_N.items():
     #   print(f"{word:8} --> {value/number_of_words: 3.2%}")
    
    
    # Create Histogram
    #list_words_dict   = list(words_dictonary_top_N.keys())
    #list_values_dict  = list(words_dictonary_top_N.values())
    list_items_dict   = list(words_dictonary_top_N.items())
    ...
    for i in xvalues:
        if yvalues[i] > 0.0:
            plt.text(xvalues[i]-0.15, yvalues[i]*(1 + 0.01), "%.2f%%" %round((yvalues[i]),2), fontsize=8)   
    plt.title(r"%s " %file_path)
    plt.suptitle(r"Histogram of words frequencies - Top %.i" %N)
    plt.xlabel(r"$Words$")
    plt.ylabel(r"$Frequencies$ [%]")
    ...
    print("-------------------------------------")
    print(f"Top {N} words:\n")
    for number, (word, value) in enumerate(words_dictonary_top_N.items()):
        print(f"{number + 1}) {word:8} --> {value/number_of_words: 3.2%}")
    print("-------------------------------------")
...
if __name__ == "__main__":
    ...
    parser.add_argument("N",        type=int, help="ranking number (the 'N' most frequent).") 
    # Opzional argument 
